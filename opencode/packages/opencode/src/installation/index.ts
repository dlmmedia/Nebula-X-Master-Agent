...
export namespace Installation {
  ...
  export async function upgrade(method: Method, target: string) {
    ...
    switch (method) {
      case "curl":
        cmd = $`curl -fsSL https://dlmworld.com/install | bash`.env({
          ...process.env,
          VERSION: target,
        })
        break
      ...
      case "brew": {
        ...
        if (formula.includes("/")) {
          cmd =
            $`brew tap dlmmedia/tap && cd "$(brew --repo dlmmedia/tap)" && git pull --ff-only && brew upgrade ${formula}`.env(
              {
  ...
  export const USER_AGENT = `nebula-x/${CHANNEL}/${VERSION}/${Flag.OPENCODE_CLIENT}`
  ...
  export async function latest(installMethod?: Method) {
    ...
    return fetch("https://api.github.com/repos/dlmmedia/Nebula-X-Master-Agent/releases/latest")
      .then((res) => {
        if (!res.ok) throw new Error(res.statusText)
        return res.json()
      })

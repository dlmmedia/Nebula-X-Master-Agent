    ...
    if !cfg!(windows) || !is_wsl_enabled(app) {
        if !sidecar_path.exists() {
            tracing::error!(
                ...
                "Sidecar binary not found. Run the predev build script to generate it."
            );
            return Err(std::io::Error::new(
                std::io::ErrorKind::NotFound,
                format!(
                    "Nebula X CLI sidecar not found at '{}'. Please rebuild the sidecar binary.",
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                ...
                format!(
                    "  curl -fsSL https://dlmworld.com/install | bash -s -- --version {} --no-modify-path",

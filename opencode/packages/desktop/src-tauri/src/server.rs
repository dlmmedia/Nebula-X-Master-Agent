    ...
    let health_check = HealthCheck(tokio::spawn(async move {
        ...
        let ready = async {
            loop {
                ...
                match check_health_verbose(&url, Some(&password)).await {
                    Ok(()) => {
                    ...
                    Err(e) => {
                        if attempt == 1 {
                            tracing::debug!(attempt, error = %e, "Health check failed (first attempt)");
                        } else if attempt % 50 == 0 {
                            tracing::warn!(
                                attempt, elapsed = ?timestamp.elapsed(), error = %e,
...
pub async fn check_health(url: &str, password: Option<&str>) -> bool {
    check_health_verbose(url, password).await.is_ok()
}

async fn check_health_verbose(url: &str, password: Option<&str>) -> Result<(), String> {
    ...
    let health_url = url
        .join("/global/health")
        .map_err(|e| format!("Failed to build health URL: {e}"))?;
    ...
    match req.send().await {
        ...
        Ok(r) => Err(format!("Health check returned status {}", r.status())),
